...
# Função para ligar o stream do ecrã e som via VLC (usando MPEG-TS)
def ligar_stream():
    stream_command = [
        r'C:\Program Files\VideoLAN\VLC\vlc.exe',  # Caminho completo para o vlc.exe
        'screen://',  # Serve a tela (ecrã)
        '--sout', '#transcode{vcodec=h264,acodec=mp3,ab=128,channels=2}:std{access=http,mux=ts,dst=:8080/}',  # Serve via HTTP com mux TS (MPEG-TS)
    ...
    print(f"Stream de ecrã e som ligado em formato MPEG-TS.")
...
def stream_video():
    ...
    return Response("http://localhost:8080/", mimetype='video/mp2t')
