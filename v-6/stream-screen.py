...
import numpy as np
import time
import vlc  # Para o áudio do sistema
...
# Caminho completo para libvlc.dll (ajustar conforme seu sistema)
libvlc_path = r'C:\Program Files\VideoLAN\VLC\libvlc.dll'
vlc_instance = vlc.Instance("--input-repeat=-1", "--fullscreen", "--no-video")
...
class StreamingApp:
    def __init__(self, master):
        self.master = master
        master.title("Aplicação de Streaming com Áudio e Vídeo")
        ...
        self.server = None
        self.audio_player = None  # Controle do áudio
    ...
    def start_audio_stream(self):
        media = vlc_instance.media_new("dshow://")
...
def update_frequency():
    ...
    if 0 <= frequency <= 100:
        CAPTURE_FREQUENCY = max(1, frequency)
        return 'OK', 200
